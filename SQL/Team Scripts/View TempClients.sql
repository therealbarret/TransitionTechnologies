CREATE VIEW TempClients AS
SELECT  FirstName AS 'First Name', LastName AS 'Last Name', sub3.StreetAddress AS 'Street Address', sub3.City, sub3.[State], sub3.[Zip Code],  
CONCAT('(', sub.AreaCodeID, ')', ' ', Number) AS 'Phone Number', Email, CASE WHEN TempClient = 1 THEN 'Yes' ELSE 'No' END AS 'Temp Client' FROM PhoneNumber
